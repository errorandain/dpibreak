...
object DiagLog {
    ...
    fun collect(context: Context): String = buildString {
        ...
        appendLine("byedpi запущен: ${ByedpiJniEngine().isRunning.value}")
        appendLine("tun2socks запущен: ${TunSocksBridge.isRunning()}")
        
        // Проверка доступности SOCKS5 прокси
        appendLine("\n=== ПРОВЕРКА SOCKS5 ===")
        val socksAvailable = checkSocks5Proxy("127.0.0.1", ByedpiJniEngine.DEFAULT_SOCKS_PORT)
        appendLine("SOCKS5 на 127.0.0.1:${ByedpiJniEngine.DEFAULT_SOCKS_PORT}: ${if (socksAvailable) "✅ доступен" else "❌ недоступен"}")
        ...
        when {
            ...
            !TunSocksBridge.isRunning() -> appendLine("⚠️ tun2socks не запущен. Туннель не работает.")
